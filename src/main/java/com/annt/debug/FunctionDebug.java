...
public class FunctionDebug {
	...
	public static void main(String[] args) {
		...
		BasicLayer l4 = new BasicLayer(1, false, new SigmoidFunction());
		SimpleNetwork network = new SimpleNetwork();
		network.addLayer(l1);
		network.addLayer(l2);
		network.addLayer(l4);
		network.initNetwork(10);
		// DoubleMatrix input = DoubleMatrix.rand(2);
		// DoubleMatrix output = DoubleMatrix.rand(5);
		...
		for (int m = 0; m < 10000; m++) {
			...
			network.updateNet(sbp.weights_updates, sbp.biass_updates, 1.5);
